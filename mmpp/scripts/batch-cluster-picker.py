...
def clusterpick (fasta_file, tree_file, init_support, boot_support, max_dist, max_size):
    ...
    outfile = fasta_file.replace('.fas', '.fas.rapidnj2_clusterPicks_list.txt')
...
files = glob('../data2/*.*_TRUE.fas')


outfile = open('../results2/cluster-picker-nj.csv', 'w')
...
for f in files:
    ...
    #treefile = f.replace('.fas', '.fas.tree')  # ML tree from FastTree2
    treefile = f.replace('.fas', '.fas.rapidnj2.nwk')  # NJ tree from RapidNJ
    #treefile = '../data/temp.nwk'  # DEBUGGING
